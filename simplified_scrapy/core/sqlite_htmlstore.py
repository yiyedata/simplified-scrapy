...
class SqliteHtmlStore(HtmlStoreBase):
    ...
    def saveHtml(self, url, html):
        ...
        conn = sqlite3.connect(self._dbPath)
        conn.cursor().execute(
            "INSERT INTO htmls (json,state,tm) VALUES (?,?,?)",
            (json.dumps({
                "url": url,
                "html": filename
            }), 0, getTimeNow()))
    ...
    def popHtml(self, state=0):
        ...
        cursor = conn.cursor().execute(
            "select id,json from htmls where state=" + str(state) +
            " limit 0,1")
        for row in cursor:
            obj = json.loads(row[1])
            obj['id'] = row[0]
            obj['html'] = io.open(self._htmlPath + obj['html'],
                                  "r",
    ...
    def updateState(self, id, state):
        conn = sqlite3.connect(self._dbPath)
        if id==0:
            conn.cursor().execute("update htmls set state=?", (state,))
        else:
            conn.cursor().execute("update htmls set state=? where id=?",
                                  (state, id))
        conn.commit()
        conn.close()
